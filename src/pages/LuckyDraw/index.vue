<template>
  <div class="page-lucky-draw">
    <van-nav-bar title="Lucky draw" left-arrow @click-left="onClickLeft" />

    <!-- 滚动号码 -->
    <div class="luckdraw-scroll mt-10">
      <ul class="bg-scroll">
    ...
    <div class="turntable">
      ...
      <ul class="awards-list">
        ...
        >
          <div v-if="key === 4" @click="handleStart" class="draw-btn">
...
<script setup lang="ts" name="LuckyDraw">
...
const move = () => {
  const timer = setTimeout(() => {
    ...
    if (award?.id && (Date.now() - time) / 1000 > minRotateTime) {
      ...
      if ((Date.now() - time) / 1000 > rotateTime && award.runId === current.value) {
        clearTimeout(timer)
        setTimeout(() => {
          isRuningLucky = false
          // 这里写停下来要执行的操作（弹出奖品框之类的）
          const getAward: AwardTypes | undefined = awards.find((v) => v.id === award.id)
          if (getAward) {
